...
function parseStepTimestamp(minutesInput, secondsInput) {
...
function addRecipeStep(initialValues = null) {
    ...
    const updateTimerState = () => {
        ...
        const stepToUpdate = timerState.steps.find(step => step.description === stepDesc && step.water === stepWater);
        if (stepToUpdate) {
        ...
        input.addEventListener("keydown", (event) => {
            ...
            if (keyboardEvent.key === "Enter") {
                ...
                if (nextInputInStep) {
                    ...
                    if (nextStep) {
                        const nextStepTime = nextStep.querySelector('.timestamp-minutes');
                        if (nextStepTime) {
                            event.preventDefault();
...
function reorderStepsInDOM() {
    ...
    const stepData = stepElements.map(step => {
        const minutesInput = step.querySelector(".timestamp-minutes");
        const secondsInput = step.querySelector(".timestamp-seconds");
        const minutes = parseInt(minutesInput?.value || "0", 10);
        const seconds = parseInt(secondsInput?.value || "0", 10);
...
function buildUrlWithValues() {
    const recipeData = {
        ...
        steps: Array.from(document.querySelectorAll(".recipe-step")).map((step) => ({
            ...
            timestampMinutes: step.querySelector(".timestamp-minutes")?.value || "0",
            timestampSeconds: step.querySelector(".timestamp-seconds")?.value || "0",
        })),
    };
...
function generateRecipeMarkdown() {
    ...
    steps.forEach((step, index) => {
        ...
        const minutes = step.querySelector(".timestamp-minutes")?.value || "0";
        const seconds = step.querySelector(".timestamp-seconds")?.value || "0";
